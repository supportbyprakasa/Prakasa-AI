...
function svcError(e, res, next) {
  if (e.status === 400) return fail(res, e.code || 'VALIDATION_ERROR', e.message, 400);
  if (e.status === 403) return fail(res, e.code || 'FORBIDDEN', e.message, 403);
  if (e.status === 404) return fail(res, e.code || 'NOT_FOUND', e.message, 404);
  return next(e);
}
