...
export default function Dashboard() {
  ...
  return <div className="hub-home fade-in">
    <div className="hub-greeting"><span className="hub-greeting__icon"><Sun size={24} strokeWidth={1.6} /></span><div><h1>{salutation}, {firstName}</h1><p>Pilih modul untuk mulai kerja.</p></div></div>
    ...
    <div className="hub-divisions">{divisions.filter((division) => visibleSections(division, sections).length).map((division) => {
      const Icon = division.icon;
      const count = visibleSections(division, sections).reduce((total, section) => total + section.items.length, 0);
      return <Link key={division.slug} to={division.to || `/hub/${division.slug}`} className="hub-division-card">
        <div className="hub-division-card__top"><span className="hub-icon"><Icon size={22} strokeWidth={1.6} /></span><ChevronRight size={17} /></div>
        <div><h2>{division.title}</h2><p>{division.description}</p></div>
        <span className="hub-count">{count} modul</span>
      </Link>;
    })}</div>
  </div>;
}
