...
(async () => {
  ...
  console.log('\n[6] notificationCenter service loads');
  try {
  ...
  console.log('\n[7] package.json script (informational)');
  try {
    const pkg = require('../../package.json');
    if (pkg.scripts && pkg.scripts['check:batch5']) pass('"check:batch5"');
    else warn('"check:batch5" not in package.json');
  } catch {
