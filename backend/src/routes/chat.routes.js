...
router.get('/rooms', requirePermission('chat.view'), ctrl.listRooms);
router.post('/rooms', requirePermission('chat.send'), validate(roomBody), ctrl.createRoom);
router.get('/rooms/:id/messages', requirePermission('chat.view'), ctrl.listMessages);
router.post('/rooms/:id/messages',
  requirePermission('chat.send'),
  validate(z.object({ body: z.string().min(1) })),
  ctrl.sendMessage);
router.post('/messages/:messageId/convert-to-task',
